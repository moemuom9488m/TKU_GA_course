...
- [x] **1-5. ⚡ 關鍵：解決 Expression Web 4 設計視窗無法正確渲染的問題**
  - **問題現象**：Expression Web 4 內建的「設計 (Design)」或「分割 (Split)」視窗中，網頁呈現為無樣式的純文字（因為該工具於 2012 年停更，內建渲染引擎太舊，不支援 Bootstrap 5 及現代 CSS）。
  - **正確作法**：忽略軟體內建的設計視窗，將其作為純程式碼編輯器使用。編輯後，務必使用 **F12（或點選瀏覽器預覽）** 在 Chrome/Edge 等現代瀏覽器中進行排版與效果確認。
